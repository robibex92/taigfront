...
const ParkingScheme = () => {
  ...
  const handleSearchCar = () => {
    // Очищаем номер от пробелов и переводим в верхний регистр
    const searchNumber = carNumber.toUpperCase().replace(/\s/g, '');
    
    
    const matchedCars = cars.filter(car => {
      const carNumber = (car.car_number || '').toUpperCase().replace(/\s/g, '');
      
      return carNumber === searchNumber;
    });
    ...
    if (matchedCars.length > 0) {
      const spots = matchedCars.map(car => {
        return cars.findIndex(c => c.id === car.id) + 1;
      });
  ...
  const handleCarClick = (car) => {
    // Если кликнули на ту же машину - закрываем карточку
    ...
    // Если кликнули на другую машину - показываем её карточку
    setSelectedCar(car);
    setFoundCars([car]);
    setShowInfo(false);
  };
